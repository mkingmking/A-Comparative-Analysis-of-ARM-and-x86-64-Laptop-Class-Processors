...
def panel_bars(x, y, w, h, title, ylabel, apple, ryzen, ymax, ticks, apple_err=None, ryzen_err=None, value_fmt="{:.1f}"):
    ...
    for bx, val, err, color, label in [
        ...
        if err:
            ...
            ey_low = yy(low)
            ey_high = yy(high)
            label_top = min(label_top, ey_high)
            ex = bx + bar_w / 2
        ...
        parts.append(text(bx + bar_w / 2, label_top - 8, value_fmt.format(val), "label"))
...
def make_energy():
    ...
    body = "\n".join(
        [
            text(width / 2, 32, "Reported Processor Energy per Completed Run", "title"),
            legend(365, 62),
            panel_bars(
                35,
                85,
...
def make_tradeoff():
    ...
    def xx(v):
        return left + (math.log10(v) - math.log10(x_min)) / (math.log10(x_max) - math.log10(x_min)) * plot_w
    ...
    for t in [0.05, 0.1, 0.2, 0.5, 1, 2, 4]:
        ty = yy(t)
        parts.append(line(left, ty, right, ty, GRID))
        parts.append(text(left - 8, ty + 4, f"{t:g}", "tick", "end"))
    parts.append(text(width / 2, 495, "Execution time (ms, log scale)", "axis"))
    parts.append(text(24, top + plot_h / 2, "Energy per run (J, log scale)", "axis", rotate=-90))
